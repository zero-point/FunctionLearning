...
function getCondition(){
  var loc = "";
  if (!conditionID) {
    console.log("Undefined condition- generating a random one");
  ...
  console.log("Condition code: "+loc[0],loc[1]);
  ...
  console.log(presentationTypes[loc[0]],functionTypes[loc[1]]); 
...
function openMsgNext() {
  ...
  if (currentMode.includes("train")) {
    ...
    text.innerHTML = "You've finished this task.<br/><br/> Click 'Next' to receive your next task.";
  }
...
function barPlot(xyValues, currentMode, expCondition){
  ...
  function clicked(d){
    ...
    if(coord>=margins && coord<=paddedHeightY){ // TODO: should 0 be allowed as a target value? (where target == 0 means coord == paddedHeightY == 500)
      ...
      chosenPoints.push(scaleY.invert(target));
      condChosenPoints.push(scaleY.invert(target));
        ...
        if(key === 32 && selection_made && finished){
          ...
          if (judgementCount != totalJudgements) {
            ...
            if (closeGuess) {
              ...
              submittedPoints.push(scaleY.invert(target));
              condSubmittedPoints.push(scaleY.invert(target));
              closeGuess = false;
              judgementCount += 1;
              if (judgementCount != totalJudgements) {
...
function introduction(){
  loadJSON('exp_input.json', function(data){
    ...
    parent.appendChild(intro);
    document.querySelector(".intro").innerHTML = "<b style=\"color:red\">DEBUG MODE <a href=\"\" onclick=\"return survey();\">Skip to survey</a></b>"+JSON.parse(data)["consentform"];
  });
}
...
function survey(){
  ...
  loadJSON('exp_input.json', function(data){
    ...
    temp += "<div class=\"wrap\"><form id=\"formy\" action=\"\">";
    for (var i=0; i<Object.keys(survey).length; i++) {
    ...
    document.querySelector(".submit").onclick = function() {
      ...
      if(missingResp){
        ...
        document.querySelector(".instructionText").innerHTML = ""; 
        document.querySelector(".container").innerHTML = "";
        document.querySelector("#main").innerHTML = '<center><h2 class="col-md-offset-3">Thank you.<br/><br/> Please copy this code as proof of your experiment completion:<br/><br/> <b>' + sessionID.substring(0, 4) + "</b></h2></center>";
      }
    };
    document.querySelector(".clear").onclick = function() {
      event.preventDefault();
      console.log("user hit clear");
      var form = document.getElementById("formy");
      for (var i=0; i<Object.keys(survey).length; i++){
        if (survey[i]["type"].includes("open")) {
...
var chunker = new ChunkWs("wss://somata.inf.ed.ac.uk/chunks/ws",function(a,b,c) {
  console.log("Done: "+a+", "+b+", "+JSON.stringify(c));
})
