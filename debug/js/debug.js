...
function getCondition() {
  ...
  if (!conditionID) {
    ...
    loc = conditionID.substring(
      (currentCondition - 1) * 2,
      currentCondition * 2
    );
  } else {
    loc = conditionID.substring(
  ...
  return [
    randPres,
    randFunc,
    presentationTypes.indexOf(randPres),
    functionTypes.indexOf(randFunc)
  ];
}
...
function openMsgNext() {
  ...
  if (currentMode.includes("train")) {
    text.innerHTML =
      "You've finished the training for this task.<br/><br/> Click 'Next' to move to the testing.";
  } else {
    text.innerHTML =
      "You've finished the experiment.<br/><br/> Click 'Next' to go to the survey.";
  }
  text.innerHTML +=
    '<br/><br/><button type="button" onclick="checkStatus()" class="btn btn-primary btn-lg nextScenarioButton">Next</button>';
...
function openMsg(closeGuess, judgementCount, currentMode) {
  ...
  if (currentMode.includes("train")) {
    if (closeGuess) {
      text.innerHTML =
        "Good job! <br/><br/> " +
        (totalJudgements - judgementCount) +
        " more to go";
    } else {
      text.innerHTML =
        "Not quite, update your choice<br/> to match the feedback in red";
    }
  } else {
...
function openAlert(msg) {
  ...
  text.innerHTML = msg;
  text.innerHTML +=
    '<br/><br/><button type="button" onclick="closeMsg()" class="btn btn-primary btn-lg nextScenarioButton">OK</button>';
  document.querySelector(".overlay-content").appendChild(text);
  document.querySelector("#myNav").style.width = "100%";
}
...
function displayInstructions(condition) {
  loadJSON("exp_input.json", function(data) {
    ...
    document.querySelector(".instructionText").innerHTML =
      parsed_data["instructions"][condition];
  });
}
...
function barPlot(xyValues, currentMode, expCondition) {
  ...
  if (currentMode.includes("train")) {
    xyValues = [
      xValue.slice(0, totalJudgements),
      yValue.slice(0, totalJudgements)
    ];
  } else {
    xyValues = [
      xValue.slice(totalJudgements, totalJudgements * 2),
      yValue.slice(totalJudgements, totalJudgements * 2)
    ];
  }
  ...
  if (debugmode) {
    var debugYRectMax = svgY
      ...
      .attr(
        "height",
        scaleY(Math.max.apply(null, yValue)) -
          scaleY(Math.min.apply(null, yValue))
      )
      .style("stroke", "green")
      .style("fill", "transparent");
    var errorMargin = svgY
      .append("svg:rect")
      .attr("x", margins)
      .attr(
        "y",
        paddedHeightY -
          scaleY(xyValues[1][judgementCount]) -
          accErrorMargin * heightY / 2
      )
      .style("stroke", "red")
      .style("fill", "transparent")
      .attr("width", paddedWidthY)
  ...
  function clicked(d) {
    ...
    if (coord >= margins && coord <= paddedHeightY) {
      // TODO: should 0 be allowed as a target value? (where target == 0 means coord == paddedHeightY == 500)
      selection_made = true;
      ...
      document.onkeyup = function(e) {
        ...
        if (key === 32 && selection_made && finished) {
          ...
          var savedAllInfo = {
            ...
            judgement: judgementCount + 1,
            x: xyValues[0][judgementCount],
            scaledX: scaleX(xyValues[0][judgementCount]),
            trueY: trueValue,
            predY: target,
            timestamp: Date.now()
          };
          ...
          delay(function() {
            if (judgementCount != totalJudgements) {
              ...
              if (
                Math.abs(trueValue - target) <= accErrorMargin * heightY / 2 ||
                currentMode.includes("test")
              ) {
                closeGuess = true;
              }
              if (closeGuess) {
                var mTimer2 = setTimeout(function() {
                  ...
                  closeGuess = false;
                  if (
                    tempListOfAttValues !== undefined &&
                    tempListOfAttValues.length !== 0
                  ) {
                  ...
                  if (judgementCount != totalJudgements) {
                    ...
                    blueXRect.attr(
                      "width",
                      scaleX(xyValues[0][judgementCount])
                    );
                    if (debugmode) {
                      errorMargin
                        .attr(
    ...
    document.onkeyup = function(e) {
      ...
      if (key === 32 && selection_made && finished && finishedTransition) {
        ...
        delay(function() {
          if (judgementCount != totalJudgements) {
            if (
              Math.abs(trueValue - target) <= accErrorMargin * height / 2 ||
              currentMode.includes("test")
            ) {
              closeGuess = true;
            }
        ...
        delay(function() {
          if (judgementCount != totalJudgements) {
            ...
            if (closeGuess) {
              ...
              if (judgementCount != totalJudgements) {
                var mTimerScatter = setTimeout(function() {
                  chart.selectAll("#feed").style("fill", "transparent");
                  chart.select("#last").remove();
                  openMsg(true, judgementCount, currentMode);
                  document.getElementById("judgementCount").innerHTML =
                    "Judgement " +
...
function checkStatus() {
  ...
  if (currentMode.includes("train")) {
    ...
    text.classList.add("unselectable");
    text.innerHTML =
      '<p>This is now the testing phase. <br/><br/>Your choices will be recorded.</p><br/><button type="button" onclick="closeDescr();" class="btn btn-primary btn-lg nextScenarioButton">Next</button>';
    document.querySelector(".overlay-content").appendChild(text);
    document.querySelector("#myNav").style.width = "100%";
    ...
    document.getElementById("judgementCount").innerHTML =
      "Judgement 1 out of " + totalJudgements;
    ...
    if (currentCondition <= totalNoOfConditions) {
      ...
      var positions = Array.apply(null, { length: 80 }).map(
        Number.call,
        Number
      );
      var xValue = positions.map(function(x) {
        return x + 1;
      });
      var yValue = xValue.map(function(x) {
        return functionDict[expCondition[1]](x);
      });
...
function openDescr(condition) {
  ...
  loadJSON("exp_input.json", function(data) {
    ...
    text.innerHTML += temp;
    text.innerHTML +=
      '<button type="button" onclick="closeDescr()" class="btn btn-primary btn-lg nextTaskButton">OK</button>';
  });
...
function introduction() {
  loadJSON("exp_input.json", function(data) {
    document.querySelector(".instructionText").innerHTML = JSON.parse(data)[
      "frontpage"
    ];
    var intro = document.createElement("div");
    ...
    parent.appendChild(intro);
    document.querySelector(".intro").innerHTML = JSON.parse(data)[
      "consentform"
    ];
  });
}
  ...
  loadJSON("exp_input.json", function(data) {
    ...
    for (var i = 0; i < Object.keys(survey).length; i++) {
      if (survey[i]["type"].includes("likert")) {
        ...
        temp +=
          '<label class="statement" id="statement' +
          i +
          '">' +
          survey[i]["question"] +
          '</label><ul class="likert">';
        for (var j = 0; j < Object.keys(likertAnswers).length; j++) {
          temp +=
            '<li><input type="radio" class="radio" name="resp' +
            i +
            '"><label>' +
            likertAnswers[j] +
            "</label></li>";
      ...
      if (survey[i]["type"].includes("closed")) {
        ...
        temp +=
          ...
          '">' +
          survey[i]["question"] +
          '</label><ul class="likert short">';
        for (var j = 0; j < Object.keys(closedAnswers).length; j++) {
          temp +=
            '<li class="check"><input type="checkbox" name="resp' +
            i +
            '"><label>' +
            closedAnswers[j] +
            "</label></li>";
      ...
      if (survey[i]["type"].includes("open")) {
        temp +=
          ...
          '">' +
          survey[i]["question"] +
          '</label><ul class="likert none">';
        temp +=
          '<textarea id="open" name="resp' +
          i +
    ...
    temp +=
      '<div class="buttons"><button class="clear">Clear</button><button type="submit" class="submit">Submit</button></div>';
    temp += "</form></div>";
    ...
    document.querySelector("#formy").onsubmit = function(e) {
      e.preventDefault();
    };
...
var functionDict = {
  ...
  periodic: function(x) {
    return Math.sin((x / 16 + 3 / 2) * Math.PI);
  }
};
...
var presentationDict = {
  ...
  "scatter-nomem": function(x, y, z) {
    scatterNoMemPlot(x, y, z);
  },
  "scatter-fullmem": function(x, y, z) {
    scatterFullMemPlot(x, y, z);
  }
};
...
var chunker = new ChunkWs("wss://somata.inf.ed.ac.uk/chunks/ws", function(
  a,
  b,
  c
) {});
